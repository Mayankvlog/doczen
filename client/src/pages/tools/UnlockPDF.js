...
export default function UnlockPDF() {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      const data = await handleToolSubmit('/pdf/unlock', formData, 'unlocked.pdf');
      setResult(data);
      if (data.blobUrl) {
  ...
  return (
    <>
    <SEO title={t('seo.unlockTitle', 'Unlock PDF Online Free - Remove Password')} description={t('seo.unlockDesc', 'Remove password protection from PDF files online. Unlock secured documents instantly - no sign-up required, 100% free.')} keywords={t('tool.unlockKeywords', 'unlock PDF, remove PDF password, decrypt PDF, PDF password remover, unlock protected PDF')} canonical="/unlock-pdf" />
