...
export default function DeletePages() {
  ...
  useEffect(() => {
    gtagEvent('tool_view', { tool_name: 'delete-pages' });
  }, []);
  ...
  const handleProcess = async () => {
    ...
    setResult(null);
    clearDownload();
    gtagEvent('tool_process', { tool_name: 'delete-pages' });

    try {
      const formData = new FormData();
      formData.append('file', file);
      formData.append('pagesToDelete', JSON.stringify(pages));
      const data = await handleToolSubmit('/pdf/delete-pages', formData, 'cleaned.pdf');
      setResult(data);
      if (data.blobUrl) {
        setDownload(data.blobUrl, data.filename || 'cleaned.pdf');
      }
      gtagEvent('tool_success', { tool_name: 'delete-pages' });
    } catch (err) {
      const msg = err.message || t('tool.deletePagesError', 'Failed to delete pages. Please try again.');
      setError(msg);
      gtagEvent('tool_error', { tool_name: 'delete-pages', error: msg });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
    <SEO title={t('seo.deletePages.title', 'Delete Pages from PDF Online Free - Fast')} description={t('seo.deletePages.desc', 'Remove unwanted pages from your PDF document online. Delete specific pages instantly - no sign-up required, 100% free.')} keywords={[ t('seo.deletePages.keywords', 'delete PDF pages, remove PDF pages, delete pages from PDF, PDF page remover, delete unwanted pages from a PDF, remove pages from a PDF by number, remove blank pages from a PDF, best way to delete PDF pages, free online PDF editor, remove specific PDF pages, delete pages by number, clear pages from PDF, remove pages from PDF online'), ...generateLongTailKeywords(120), ...generateGeoKeywords(120), ...generateGhostKeywords(120) ].join(', ')} canonical="/delete-pages" />
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-primary-50/30 to-gray-50 py-12 px-4">
      <div className="max-w-3xl mx-auto">
        <div className="text-center mb-10">
          ...
          <h1 className="text-3xl md:text-4xl font-bold text-gray-900">{t('seo.deletePages.title', 'Delete Pages from PDF Online Free')}</h1>
